...
namespace KBS.KBS.CMSV3.MasterData.Assortment
{
    public partial class ItemSearch : System.Web.UI.Page
    {
        ...
        {
             Session["ItemIDExAssortment"] = ASPxGridViewItemVariant.GetRowValues(Convert.ToInt32(e.Parameters), "ITEM ID").ToString();
             Session["VariantIDExAssortment"] = ASPxGridViewItemVariant.GetRowValues(Convert.ToInt32(e.Parameters), "VARIANT ID").ToString();

            if (Page.IsCallback)
                ASPxWebControl.RedirectOnCallback("AssortmentManagement.aspx");
            else
                Response.Redirect("AssortmentManagement.aspx");
        ...
        protected void ASPxButtonOK_Click(object sender, EventArgs e)
        {

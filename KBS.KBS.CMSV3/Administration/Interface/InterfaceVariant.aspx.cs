...
{
    public partial class InterfaceVariant : System.Web.UI.Page
    {
        ...
        {
            CMSfunction.ExecuteSPProcessVariant();
            LoadDataGrid();
        ...
        {
            if (FileUploadVariant.HasFile == false)
            {
                // No file uploaded!
            ...
            else
            {
                try
                {
                    ...
                    filePath = CMSfunction.getIncomingDirectory() + FileUploadVariant.FileName;

                    FileUploadVariant.SaveAs(filePath);
        ...
        {
            Session["InterfaceVariantRowID"] = ASPxGridViewHeader.GetRowValues(Convert.ToInt32(e.Parameters), "ROWID").ToString();

            if (Page.IsCallback)
                ASPxWebControl.RedirectOnCallback("InterfaceVariantDetail.aspx");
            else
                Response.Redirect("InterfaceVariantDetail.aspx");
        }
    }
}